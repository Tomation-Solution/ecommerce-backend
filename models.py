...
class Customers(db.Model):
    ...
    address = db.relationship('Address', backref=db.backref('customer', lazy=True), cascade="all, delete-orphan")
    order = db.relationship('Orders', backref=db.backref('customer'), lazy=True)
...
class Products(db.Model):
    ...
    orderdetails = db.relationship('OrderDetails', backref=db.backref('product', lazy=True))
...
class Categories(db.Model):
    ...
    date_created = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    product = db.relationship('Products', backref=db.backref('category', lazy=True), cascade="all, delete-orphan")
...
class Orders(db.Model):
    ...
    customer_id = db.Column(db.Integer, db.ForeignKey('customers.customer_id'), nullable=False)
    orderdetails = db.relationship('OrderDetails', backref=db.backref('order', lazy=True), cascade="all, delete-orphan")
